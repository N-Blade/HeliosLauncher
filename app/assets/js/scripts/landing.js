  ...
  if (authUser != null) {
    ...
    if (authUser.uuid != null) {
      document.getElementById(
        "avatarContainer"
      ).style.backgroundImage = `url('https://www.northernblade.ru/forums/image.php?u=${authUser.uuid}')`;
    }
  }
...
function dlAsync(login = true) {
  ...
  aEx = cp.fork(
    ...
    ["AssetGuard", ConfigManager.getCommonDirectory(), remote.app.getVersion()],
    {
      env: forkEnv,
  ...
  aEx.on("message", (m) => {
    if (m.context === "validate") {
      ...
      if (login && allGood) {
        ...
        try {
          // Build Game process.
          pb.addErrorListener(gameGlobalErrorListener).addCloseListener(
            (code, signal) => {
              toggleLaunchArea(false);
              pb = null;
            }
